...
uint8 Comp_Fing_In_initVar = 0u;

/* Internal functions definitoin */
static void Comp_Fing_In_trimAdjust(uint8 nibble) ;

/* static Comp_Fing_In_backupStruct  Comp_Fing_In_backup; */
#if (CY_PSOC5A)
    static Comp_Fing_In_LOWPOWER_BACKUP_STRUCT  Comp_Fing_In_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_Fing_In_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_Fing_In_Init
********************************************************************************
*
* Summary:
...
void Comp_Fing_In_Init(void) 
{
    ...
    Comp_Fing_In_SetSpeed(Comp_Fing_In_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_Fing_In_DEFAULT_HYSTERESIS == 0u )
        Comp_Fing_In_CR |= Comp_Fing_In_HYST_OFF;
    #else
        Comp_Fing_In_CR &= (uint8)(~Comp_Fing_In_HYST_OFF);
    #endif /* Comp_Fing_In_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_Fing_In_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_Fing_In_CR &= (uint8)(~Comp_Fing_In_PWRDWN_OVRD);
        #else 
            Comp_Fing_In_CR |= Comp_Fing_In_PWRDWN_OVRD;
        #endif /* Comp_Fing_In_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_Fing_In_CR |= Comp_Fing_In_MX_AO;

    /* Set default sync */
    Comp_Fing_In_CLK &= (uint8)(~Comp_Fing_In_SYNCCLK_MASK);
    #if ( Comp_Fing_In_DEFAULT_BYPASS_SYNC == 0u )
        Comp_Fing_In_CLK |= Comp_Fing_In_SYNC_CLK_EN;
    #else
        Comp_Fing_In_CLK |= Comp_Fing_In_BYPASS_SYNC;
    #endif /* Comp_Fing_In_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_Fing_In_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_Fing_In_PWRMGR |= Comp_Fing_In_ACT_PWR_EN;
    Comp_Fing_In_STBY_PWRMGR |= Comp_Fing_In_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_Fing_In_restoreReg == 1u)
        {
            Comp_Fing_In_CR = Comp_Fing_In_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_Fing_In_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Comp_Fing_In_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_Fing_In_initVar: Is modified when this function is called for the 
...
void Comp_Fing_In_Start(void) 
{
    ...
    if ( Comp_Fing_In_initVar == 0u )
    {
        Comp_Fing_In_Init();
        
        Comp_Fing_In_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Comp_Fing_In_Stop
********************************************************************************
*
* Summary:
...
void Comp_Fing_In_Stop(void) 
{
    ...
    Comp_Fing_In_PWRMGR &= (uint8)(~Comp_Fing_In_ACT_PWR_EN);
    Comp_Fing_In_STBY_PWRMGR &= (uint8)(~Comp_Fing_In_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_Fing_In_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_Fing_In_lowPowerBackup.compCRReg = Comp_Fing_In_CR;
        Comp_Fing_In_CR = Comp_Fing_In_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_Fing_In_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_Fing_In_SetSpeed(uint8 speed) 
{
    ...
    Comp_Fing_In_CR = (Comp_Fing_In_CR & (uint8)(~Comp_Fing_In_PWR_MODE_MASK)) |
                           (speed & Comp_Fing_In_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_Fing_In_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Fing_In_TR = Comp_Fing_In_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Fing_In_TR0 = Comp_Fing_In_HS_TRIM_TR0;
            Comp_Fing_In_TR1 = Comp_Fing_In_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Fing_In_TR = Comp_Fing_In_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Fing_In_TR0 = Comp_Fing_In_LS_TRIM_TR0;
            Comp_Fing_In_TR1 = Comp_Fing_In_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_Fing_In_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_Fing_In_WRK & Comp_Fing_In_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_Fing_In_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_Fing_In_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_Fing_In_WRK & Comp_Fing_In_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Fing_In_TR |= Comp_Fing_In_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Fing_In_TR0 |= Comp_Fing_In_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Fing_In_TR |= Comp_Fing_In_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Fing_In_TR1 |= Comp_Fing_In_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_Fing_In_WRK & Comp_Fing_In_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_Fing_In_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_Fing_In_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_Fing_In_SW0;
    tmpSW2 = Comp_Fing_In_SW2;
    tmpSW3 = Comp_Fing_In_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_Fing_In_SW0 = 0x00u;
    Comp_Fing_In_SW2 = 0x00u;
    Comp_Fing_In_SW3 = tmpSW3 & (uint8)(~Comp_Fing_In_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Comp_Fing_In_CR;
    Comp_Fing_In_CR |= (Comp_Fing_In_CAL_ON | Comp_Fing_In_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_Fing_In_TR = Comp_Fing_In_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_Fing_In_TR0 = Comp_Fing_In_DEFAULT_CMP_TRIM;
        Comp_Fing_In_TR1 = Comp_Fing_In_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_Fing_In_CR & Comp_Fing_In_PWR_MODE_MASK) == Comp_Fing_In_PWR_MODE_FAST)
    {
        Comp_Fing_In_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_Fing_In_trimAdjust(1u);
		Comp_Fing_In_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Comp_Fing_In_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Comp_Fing_In_SW0 = tmpSW0;
    Comp_Fing_In_SW2 = tmpSW2;
    Comp_Fing_In_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_Fing_In_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_Fing_In_TR1 << 8u) | (Comp_Fing_In_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_Fing_In_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_Fing_In_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_Fing_In_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_Fing_In_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
