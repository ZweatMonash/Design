...
/*******************************************************************************
* Function Name: BCD_ClkInternal_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_Start(void) 
{
    /* Set the bit to enable the clock. */
    BCD_ClkInternal_CLKEN |= BCD_ClkInternal_CLKEN_MASK;
	BCD_ClkInternal_CLKSTBY |= BCD_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    BCD_ClkInternal_CLKEN &= (uint8)(~BCD_ClkInternal_CLKEN_MASK);
	BCD_ClkInternal_CLKSTBY &= (uint8)(~BCD_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_StopBlock(void) 
{
    if ((BCD_ClkInternal_CLKEN & BCD_ClkInternal_CLKEN_MASK) != 0u)
    {
...
#if defined(BCD_ClkInternal__CFG3)
        CLK_DIST_AMASK = BCD_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BCD_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BCD_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BCD_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BCD_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        BCD_ClkInternal_CLKSTBY &= (uint8)(~BCD_ClkInternal_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = BCD_ClkInternal_GetSourceRegister();
    uint16 oldDivider = BCD_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BCD_ClkInternal_CLKEN & BCD_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(BCD_ClkInternal_DIV_PTR, clkDivider);
                BCD_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                BCD_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(BCD_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BCD_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 BCD_ClkInternal_GetDividerRegister(void) 
{
    return CY_GET_REG16(BCD_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_SetModeRegister(uint8 modeBitMask) 
{
    BCD_ClkInternal_MOD_SRC |= modeBitMask & (uint8)BCD_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_ClearModeRegister(uint8 modeBitMask) 
{
    BCD_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BCD_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BCD_ClkInternal_GetModeRegister(void) 
{
    return BCD_ClkInternal_MOD_SRC & (uint8)(BCD_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = BCD_ClkInternal_GetDividerRegister();
    uint8 oldSrc = BCD_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BCD_ClkInternal_MOD_SRC |= CYCLK_SSS;
        BCD_ClkInternal_MOD_SRC =
            (BCD_ClkInternal_MOD_SRC & (uint8)(~BCD_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BCD_ClkInternal_GetSourceRegister(void) 
{
    return BCD_ClkInternal_MOD_SRC & BCD_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BCD_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BCD_ClkInternal_SetPhaseRegister(uint8 clkPhase) 
{
    BCD_ClkInternal_PHASE = clkPhase & BCD_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BCD_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BCD_ClkInternal_GetPhaseRegister(void) 
{
    return BCD_ClkInternal_PHASE & BCD_ClkInternal_PHASE_MASK;
}
