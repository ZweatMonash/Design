...
#include "EEPROM_Group.h"


/*******************************************************************************
* Function Name: EEPROM_Group_Enable
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Group_Start
********************************************************************************
*
* Summary:
*  Starts EEPROM.
...
/*******************************************************************************
* Function Name: EEPROM_Group_Stop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Group_WriteByte
********************************************************************************
*
* Summary:
...
*  call EEPROM_Group_UpdateTemperature() function if the temperature of the
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    EEPROM_Group_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: EEPROM_Group_ReadByte
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Group_UpdateTemperature
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Group_EraseSector
********************************************************************************
*
* Summary:
...
cystatus EEPROM_Group_EraseSector(uint8 sectorNumber) 
{
    ...
    if(sectorNumber < (uint8) EEPROM_Group_SECTORS_NUMBER)
    {
...
/*******************************************************************************
* Function Name: EEPROM_Group_Write
********************************************************************************
*
* Summary:
...
*  EEPROM_Group_UpdateTemperature() function if the temperature of the
...
/*******************************************************************************
* Function Name: EEPROM_Group_StartWrite
********************************************************************************
*
* Summary:
...
*  EEPROM_Group_Query(). EEPROM_Group_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until EEPROM_Group_Query() detects that
...
*  you should call EEPROM_Group_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: EEPROM_Group_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with EEPROM_Group_Query().
*  EEPROM_Group_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until EEPROM_Group_Query() detects that the erase is
...
/*******************************************************************************
* Function Name: EEPROM_Group_Query
********************************************************************************
*
* Summary:
*  Checks the status of an earlier call to EEPROM_Group_StartWrite() or
*  EEPROM_Group_StartErase().
...
/*******************************************************************************
* Function Name: EEPROM_Group_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                EEPROM_Group_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
