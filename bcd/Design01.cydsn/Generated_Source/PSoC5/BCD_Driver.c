...
#include <BCD_Driver_DMA_Com_dma.h>
#include <BCD_Driver_DMA_Seg_dma.h>
#if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
    #include <BCD_Driver_DMA_BC_dma.h>
#endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */

/* Function prototypes */
static void BCD_Driver_DmaInit(void) ;
static void BCD_Driver_DmaEnable(void) ;
static void BCD_Driver_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(BCD_Driver_COM_DRIVE == BCD_Driver_HIGH)
    static uint8 BCD_Driver_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 BCD_Driver_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (BCD_Driver_COM_DRIVE == BCD_Driver_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 BCD_Driver_segLBuffer[BCD_Driver_NUM_COM];
#if (BCD_Driver_NUM_SEG > 8u) 
    uint8 BCD_Driver_segMBuffer[BCD_Driver_NUM_COM];
#endif /* (BCD_Driver_NUM_SEG > 8u) */
#if (BCD_Driver_NUM_SEG > 16u) 
    uint8 BCD_Driver_segHBuffer[BCD_Driver_NUM_COM];
#endif /* (BCD_Driver_NUM_SEG > 16u) */

#if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 BCD_Driver_brightnessReg[BCD_Driver_NUM_COM];
#endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 BCD_Driver_dmaComChannel;
static uint8 BCD_Driver_dmaComTd[BCD_Driver_TWO_NUM_COM];

static uint8 BCD_Driver_dmaSegChannel;
static uint8 BCD_Driver_dmaSegLTd[BCD_Driver_NUM_COM];

#if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
    static uint8 BCD_Driver_dmaBCChannel;
    static uint8 BCD_Driver_dmaBCTd;
#endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */

uint8 BCD_Driver_initVar = 0u;
uint8 BCD_Driver_enableVar = 0u;


/*******************************************************************************
* Function Name: BCD_Driver_DmaInit
********************************************************************************
*
* Summary: 
...
static void BCD_Driver_DmaInit(void) 
{
    ...
    #if (BCD_Driver_NUM_SEG > 8u)
        static uint8 BCD_Driver_dmaSegMTd[BCD_Driver_NUM_COM];
    #endif /* (BCD_Driver_NUM_SEG > 8u) */
    #if (BCD_Driver_NUM_SEG > 16u)
        static uint8 BCD_Driver_dmaSegHTd[BCD_Driver_NUM_COM];
    #endif /* (BCD_Driver_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    BCD_Driver_dmaComChannel = BCD_Driver_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(BCD_Driver_SRAM_BASE), HI16(BCD_Driver_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(BCD_Driver_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < BCD_Driver_TWO_NUM_COM; i++)
        {
            BCD_Driver_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (BCD_Driver_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaComTd[i], 1u, 
                                                BCD_Driver_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | BCD_Driver_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaComTd[i], 1u,
                                                BCD_Driver_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(BCD_Driver_dmaComTd[BCD_Driver_TWO_NUM_COM - 1u], 
                                        1u, BCD_Driver_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < BCD_Driver_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(BCD_Driver_dmaComTd[i],
                                        LO16((uint32) &BCD_Driver_comBuffer[i]),
                                        LO16((uint32) BCD_Driver_COM_CONTROL_PTR));
        }    
    }
    ...
    BCD_Driver_dmaSegChannel = BCD_Driver_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(BCD_Driver_SRAM_BASE), HI16(BCD_Driver_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(BCD_Driver_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < BCD_Driver_NUM_COM; i++)
        {
            BCD_Driver_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (BCD_Driver_NUM_SEG > 8u)
                BCD_Driver_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
            #if (BCD_Driver_NUM_SEG > 16u)
                BCD_Driver_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (BCD_Driver_NUM_COM - 1u); i++)
        {
            #if (BCD_Driver_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegLTd[i], 1u, BCD_Driver_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegLTd[i], 1u, BCD_Driver_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (BCD_Driver_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegMTd[i], 1u, BCD_Driver_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegMTd[i], 1u, BCD_Driver_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegHTd[i], 1u, BCD_Driver_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
                #endif /* BCD_Driver_NUM_SEG <= 16u */
            #endif /* BCD_Driver_NUM_SEG <= 8u */
        }
        ...
        #if (BCD_Driver_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegLTd[BCD_Driver_NUM_COM - 1u],
                                            1u, BCD_Driver_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegLTd[BCD_Driver_NUM_COM - 1u],
                                            1u, BCD_Driver_dmaSegMTd[BCD_Driver_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (BCD_Driver_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegMTd[BCD_Driver_NUM_COM - 1u],
                                                1u, BCD_Driver_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegMTd[BCD_Driver_NUM_COM - 1u],
                                                1u, BCD_Driver_dmaSegHTd[BCD_Driver_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(BCD_Driver_dmaSegHTd[BCD_Driver_NUM_COM - 1u],
                                                1u, BCD_Driver_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | BCD_Driver_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (BCD_Driver_NUM_SEG <= 16u) */
        #endif /* (BCD_Driver_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < BCD_Driver_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(BCD_Driver_dmaSegLTd[i],
                                            LO16((uint32) &BCD_Driver_segLBuffer[i]),
                                            LO16((uint32) BCD_Driver_SEG_L_CONTROL_PTR));
            #if (BCD_Driver_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(BCD_Driver_dmaSegMTd[i],
                                            LO16((uint32) &BCD_Driver_segMBuffer[i]),
                                            LO16((uint32) BCD_Driver_SEG_M_CONTROL_PTR));
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
            #if (BCD_Driver_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(BCD_Driver_dmaSegHTd[i], 
                                            LO16((uint32) &BCD_Driver_segHBuffer[i]),
                                            LO16((uint32) BCD_Driver_SEG_H_CONTROL_PTR));
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        BCD_Driver_dmaBCChannel = BCD_Driver_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(BCD_Driver_SRAM_BASE), HI16(BCD_Driver_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(BCD_Driver_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            BCD_Driver_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(BCD_Driver_dmaBCTd, BCD_Driver_NUM_COM,
                                        BCD_Driver_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(BCD_Driver_dmaBCTd, LO16((uint32) &BCD_Driver_brightnessReg[0u]),
                                                               LO16((uint32)BCD_Driver_COMPARE_PTR));
        }
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
}
...
/*******************************************************************************
* Function Name: BCD_Driver_DmaEnable
********************************************************************************
*
* Summary: 
...
static void BCD_Driver_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(BCD_Driver_dmaComChannel, BCD_Driver_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(BCD_Driver_dmaSegChannel, BCD_Driver_dmaSegLTd[0u]);
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
        (void) CyDmaChSetInitialTd(BCD_Driver_dmaBCChannel, BCD_Driver_dmaBCTd);
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(BCD_Driver_dmaComChannel, 1u);
    (void) CyDmaChEnable(BCD_Driver_dmaSegChannel, 1u);
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
        (void) CyDmaChEnable(BCD_Driver_dmaBCChannel, 1u);
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
}


/*******************************************************************************
* Function Name: BCD_Driver_DmaDisable
********************************************************************************
*
* Summary: 
...
static void BCD_Driver_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[BCD_Driver_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = BCD_Driver_dmaComChannel;
    dmaChannelList[1u] = BCD_Driver_dmaSegChannel;
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
        dmaChannelList[2u] = BCD_Driver_dmaBCChannel;
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */

    for(i = 0u; i < BCD_Driver_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        BCD_Driver_Com_Driver_Write(BCD_Driver_CLEAR);
...
{
    static uint8 BCD_Driver_dmaInitDone = 0u;
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
        uint8 i;
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
    
    /* Start fresh by clearing the display */
    BCD_Driver_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == BCD_Driver_dmaInitDone)
    {
        BCD_Driver_DmaInit();
        BCD_Driver_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(BCD_Driver_NUM_COM); i++)
        {
            BCD_Driver_brightnessReg[i] = BCD_Driver_INIT_BRIGHT;
        }
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
}
...
/*******************************************************************************
* Function Name: BCD_Driver_Enable
********************************************************************************
*
* Summary: 
...
{
    /* Enable the DMAs */
    BCD_Driver_DmaEnable();
 
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
       /* Initialize to zero */
       BCD_Driver_COUNTER_REG = 0x00u;
       BCD_Driver_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       BCD_Driver_CONTROL_REG = BCD_Driver_CTRL_ENABLE;
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
    
    BCD_Driver_enableVar = BCD_Driver_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: BCD_Driver_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == BCD_Driver_initVar)
    {
        BCD_Driver_Init();
        BCD_Driver_initVar = 1u;
    }
...
void BCD_Driver_Stop(void) 
{
    #if (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED)
        /* Disable the PWM */
        BCD_Driver_CONTROL_REG = BCD_Driver_CTRL_DISABLE;
    #endif /* (BCD_Driver_BRIGHTNESS_EN == BCD_Driver_ENABLED) */
    
    BCD_Driver_DmaDisable();
    BCD_Driver_ClearDisplayAll();
    
    BCD_Driver_enableVar  = BCD_Driver_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: BCD_Driver_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to BCD_Driver_segLBuffer,
*  BCD_Driver_segMBuffer, BCD_Driver_segHBuffer.
*
* Parameters:
...
void BCD_Driver_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        relPosition = position;
        #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
            BCD_Driver_segLBuffer[relPosition] = value;
        #else
            BCD_Driver_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
    }    
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
        #if (BCD_Driver_NUM_SEG > 8u)
            relPosition = position - BCD_Driver_NUM_COM;
            #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                BCD_Driver_segMBuffer[relPosition] = value;
            #else
                BCD_Driver_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
    }
    else
    {
        #if (BCD_Driver_NUM_SEG > 16u)
            relPosition = position - BCD_Driver_TWO_NUM_COM;
            if(relPosition < BCD_Driver_NUM_COM)
            {
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segHBuffer[relPosition] = value;
                #else
                    BCD_Driver_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
            }
...
/*******************************************************************************
* Function Name: BCD_Driver_SetRC
********************************************************************************
*
* Summary:
...
void BCD_Driver_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < BCD_Driver_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                BCD_Driver_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                BCD_Driver_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    BCD_Driver_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    BCD_Driver_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_ClearRC
********************************************************************************
*
* Summary:
...
void BCD_Driver_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < BCD_Driver_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                BCD_Driver_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                BCD_Driver_segLBuffer[column] |= rowSelect;
            #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    BCD_Driver_segMBuffer[column] |= rowSelect;
                #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    BCD_Driver_segHBuffer[column] |= rowSelect;
                #endif /* BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH */
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_ToggleRC
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BCD_Driver_GetRC
********************************************************************************
*
* Summary:
...
uint8 BCD_Driver_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < BCD_Driver_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)BCD_Driver_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (BCD_Driver_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)BCD_Driver_segMBuffer[column] >> rowSeg);
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
        }
        else
        {
            #if (BCD_Driver_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)BCD_Driver_segHBuffer[column] >> rowSeg);
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: BCD_Driver_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        BCD_Driver_segLBuffer[relPosition] = BCD_Driver_CLEAR;
    }    
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
        #if (BCD_Driver_NUM_SEG > 8u)
            relPosition = position - BCD_Driver_NUM_COM;
            BCD_Driver_segMBuffer[relPosition] = BCD_Driver_CLEAR;
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < BCD_Driver_NUM_COM)
            {
                BCD_Driver_segHBuffer[relPosition] = BCD_Driver_CLEAR;
            }
        #endif /* (BCD_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: BCD_Driver_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void BCD_Driver_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < BCD_Driver_NUM_COM; position++)
    {
        BCD_Driver_segLBuffer[position] = BCD_Driver_CLEAR; 
        
        #if (BCD_Driver_NUM_SEG > 8u) 
            BCD_Driver_segMBuffer[position] = BCD_Driver_CLEAR;
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
        
        #if (BCD_Driver_NUM_SEG > 16u)
            BCD_Driver_segHBuffer[position] = BCD_Driver_CLEAR;
        #endif /* (BCD_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: BCD_Driver_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        choiceReg = BCD_Driver_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
        choiceReg = BCD_Driver_SEG_REG_MID;
        relPosition = position - BCD_Driver_NUM_COM;
    }
    else
    {
        choiceReg = BCD_Driver_SEG_REG_HIGH;
        relPosition = position - BCD_Driver_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= BCD_Driver_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = BCD_Driver_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = BCD_Driver_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = BCD_Driver_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == BCD_Driver_ZERO_PAD)
    {
        if(numberSign == BCD_Driver_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = BCD_Driver_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != BCD_Driver_ZERO_7CHAR)
            {
                if(numberSign == BCD_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = BCD_Driver_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = BCD_Driver_EncodeChar7Seg('-');
                    }
                }
    ...
    if(alignment == BCD_Driver_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != BCD_Driver_CLEAR)
            {
                if(relPosition < BCD_Driver_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case BCD_Driver_SEG_REG_LOW :
                            BCD_Driver_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case BCD_Driver_SEG_REG_MID :
                            #if (BCD_Driver_NUM_SEG > 8u)
                                BCD_Driver_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (BCD_Driver_NUM_SEG > 8u) */
                            break;
                        case BCD_Driver_SEG_REG_HIGH :
                            #if (BCD_Driver_NUM_SEG > 16u)
                                BCD_Driver_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (BCD_Driver_NUM_SEG > 16u) */
                            break;
                        default :
                            BCD_Driver_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < BCD_Driver_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case BCD_Driver_SEG_REG_LOW :
                        BCD_Driver_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case BCD_Driver_SEG_REG_MID :
                        #if (BCD_Driver_NUM_SEG > 8u)
                            BCD_Driver_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (BCD_Driver_NUM_SEG > 8u) */
                        break;
                    case BCD_Driver_SEG_REG_HIGH :
                        #if (BCD_Driver_NUM_SEG > 16u)
                            BCD_Driver_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (BCD_Driver_NUM_SEG > 16u) */
                        break;
                    default :
                        BCD_Driver_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: BCD_Driver_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & BCD_Driver_NIBBLE_MASK);
        bufArray[i-1u] = BCD_Driver_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> BCD_Driver_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < BCD_Driver_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                BCD_Driver_segLBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (BCD_Driver_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - BCD_Driver_NUM_COM;
            for(relPosition = position - BCD_Driver_NUM_COM;
                relPosition < BCD_Driver_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    BCD_Driver_segMBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
    ...
    {
        #if (BCD_Driver_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - BCD_Driver_TWO_NUM_COM;
            for(relPosition = position - BCD_Driver_TWO_NUM_COM;
                relPosition < BCD_Driver_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    BCD_Driver_segHBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (BCD_Driver_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: BCD_Driver_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        BCD_Driver_segLBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character);
    }    
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
        #if (BCD_Driver_NUM_SEG > 8u) 
            relPosition = position - BCD_Driver_NUM_COM;
            BCD_Driver_segMBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character);
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < BCD_Driver_NUM_COM)
            {
                BCD_Driver_segHBuffer[relPosition] = BCD_Driver_EncodeChar7Seg(character);
            }
        #endif /* (BCD_Driver_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: BCD_Driver_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            BCD_Driver_segLBuffer[relPosition] = BCD_Driver_EncodeNumber7Seg(digit);
        }    
        else if (position < BCD_Driver_TWO_NUM_COM)
        {
            #if (BCD_Driver_NUM_SEG > 8u)
                relPosition = position - BCD_Driver_NUM_COM;
                BCD_Driver_segMBuffer[relPosition] = BCD_Driver_EncodeNumber7Seg(digit);
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
        } 
        else
        {
            #if (BCD_Driver_NUM_SEG > 16u)
                relPosition = position - BCD_Driver_TWO_NUM_COM;
                if(relPosition < BCD_Driver_TWO_NUM_COM)
                {
                    BCD_Driver_segHBuffer[relPosition] = BCD_Driver_EncodeNumber7Seg(digit);
                }
            #endif /* (BCD_Driver_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BCD_Driver_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = BCD_Driver_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == BCD_Driver_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = BCD_Driver_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != BCD_Driver_ZERO_14CHAR)
            {
                if(numberSign == BCD_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = BCD_Driver_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = BCD_Driver_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
            ...
            else
            {
                ...
                {
                    /* Turn off the display */
                    bufArray[i] = BCD_Driver_CLEAR_16;
                    zeroCount++;
                }
            }    
        }
    ...
    if(alignment == BCD_Driver_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != BCD_Driver_CLEAR_16)
            {
                if(relPosition < BCD_Driver_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    BCD_Driver_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (BCD_Driver_NUM_SEG > 8u)
                    BCD_Driver_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (BCD_Driver_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < BCD_Driver_NUM_COM)
            {
                BCD_Driver_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (BCD_Driver_NUM_SEG > 8u)
                BCD_Driver_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: BCD_Driver_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < BCD_Driver_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = BCD_Driver_EncodeChar14Seg(character[i]);
            BCD_Driver_segLBuffer[currPosition] = LO8(charDisplay);
            #if (BCD_Driver_NUM_SEG > 8u)
            BCD_Driver_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        charDisplay = BCD_Driver_EncodeChar14Seg(character);
        BCD_Driver_segLBuffer[position] = LO8(charDisplay);
    #if (BCD_Driver_NUM_SEG > 8u)
        BCD_Driver_segMBuffer[position] = HI8(charDisplay);
    #endif /* (BCD_Driver_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: BCD_Driver_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < BCD_Driver_NUM_COM))
    {
        charDisplay = BCD_Driver_EncodeNumber14Seg(digit);
        BCD_Driver_segLBuffer[position] = LO8(charDisplay);
    #if (BCD_Driver_NUM_SEG > 8u)
        BCD_Driver_segMBuffer[position] = HI8(charDisplay);
    #endif /* (BCD_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: BCD_Driver_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: BCD_Driver_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = BCD_Driver_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == BCD_Driver_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = BCD_Driver_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != BCD_Driver_ZERO_16CHAR)
            {
                if(numberSign == BCD_Driver_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = BCD_Driver_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = BCD_Driver_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: BCD_Driver_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: BCD_Driver_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < BCD_Driver_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = BCD_Driver_EncodeChar16Seg(character[i]);
            BCD_Driver_segLBuffer[currPosition] = LO8(charDisplay);
            #if (BCD_Driver_NUM_SEG > 8u)
                BCD_Driver_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (BCD_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: BCD_Driver_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        charDisplay = BCD_Driver_EncodeChar16Seg(character);
        BCD_Driver_segLBuffer[position] = LO8(charDisplay);
        #if (BCD_Driver_NUM_SEG > 8u)
            BCD_Driver_segMBuffer[position] = HI8(charDisplay);
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: BCD_Driver_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < BCD_Driver_NUM_COM))
    {
        charDisplay = BCD_Driver_EncodeNumber16Seg(digit);
        BCD_Driver_segLBuffer[position] = LO8(charDisplay);
        #if (BCD_Driver_NUM_SEG > 8u)
            BCD_Driver_segMBuffer[position] = HI8(charDisplay);
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: BCD_Driver_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: BCD_Driver_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    value = BCD_Driver_DEC_POINT_VALUE;
    ...
    if(position < BCD_Driver_NUM_COM)
    {
        ...
        {
            #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                BCD_Driver_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                BCD_Driver_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH) */
        }
    ...
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
            ...
            {
                #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                    BCD_Driver_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    BCD_Driver_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < BCD_Driver_NUM_COM)
            {
                ...
                {
                    #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
                        BCD_Driver_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        BCD_Driver_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH) */
                }
...
/*******************************************************************************
* Function Name: BCD_Driver_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 BCD_Driver_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(BCD_Driver_segLBuffer[relPosition]) >> BCD_Driver_DEC_POINT);
    } 
    else if (position < BCD_Driver_TWO_NUM_COM)
    {
        #if (BCD_Driver_NUM_SEG > 8u) 
            relPosition = position - BCD_Driver_NUM_COM;
            decPointVal = (uint8)((uint8)(BCD_Driver_segMBuffer[relPosition]) >> BCD_Driver_DEC_POINT);
        #endif /* (BCD_Driver_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < BCD_Driver_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(BCD_Driver_segHBuffer[relPosition]) >> BCD_Driver_DEC_POINT);
            }
        #endif /* (BCD_Driver_NUM_SEG > 16u) */
    }
    
    #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
        value = (uint8)(decPointVal & BCD_Driver_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & BCD_Driver_DEC_POINT_MASK);
    #endif /* (BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH) */
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 BCD_Driver_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
        static const uint8 CYCODE BCD_Driver_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  BCD_Driver_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & BCD_Driver_NIBBLE_MASK);
    
    valHex = BCD_Driver_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 BCD_Driver_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(BCD_Driver_SEG_DRIVE == BCD_Driver_HIGH)
        static const uint8 CYCODE BCD_Driver_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & BCD_Driver_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> BCD_Driver_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > BCD_Driver_SEG_ASCII_LIM_LO) && (valHigh < BCD_Driver_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - BCD_Driver_SEG_ASCII_OFFSET;
        valHex = BCD_Driver_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = BCD_Driver_CLEAR;
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 BCD_Driver_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE BCD_Driver_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE BCD_Driver_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)BCD_Driver_14SegDigitsHigh[valNum] << BCD_Driver_SHIFT_BYTE);
    valLow = (uint16)BCD_Driver_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 BCD_Driver_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE BCD_Driver_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE BCD_Driver_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > BCD_Driver_SEG_ASCII_LIM_LO) && (valHigh < BCD_Driver_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)BCD_Driver_14SegASCIIHigh[valHigh][valLow] << BCD_Driver_SHIFT_BYTE);
        valHexLow = (uint16)BCD_Driver_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = BCD_Driver_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 BCD_Driver_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE BCD_Driver_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE BCD_Driver_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)BCD_Driver_16SegDigitsHigh[valNum] << BCD_Driver_SHIFT_BYTE);
    valLow = (uint16)BCD_Driver_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: BCD_Driver_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 BCD_Driver_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE BCD_Driver_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE BCD_Driver_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > BCD_Driver_SEG_ASCII_LIM_LO) && (valHigh < BCD_Driver_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)BCD_Driver_16SegASCIIHigh[valHigh][valLow] << BCD_Driver_SHIFT_BYTE);
        valHexLow = (uint16)BCD_Driver_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: BCD_Driver_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(BCD_Driver_COM_DRIVE == BCD_Driver_HIGH)
        static const uint8 CYCODE BCD_Driver_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE BCD_Driver_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        BCD_Driver_comBuffer[(uint8)((uint8)position << 1u) + 1u] = BCD_Driver_CLEAR;
    }
    else
    {
        /* Restore the commons */
        BCD_Driver_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = BCD_Driver_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    BCD_Driver_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: BCD_Driver_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(BCD_Driver_brightnessReg[position]);
}
