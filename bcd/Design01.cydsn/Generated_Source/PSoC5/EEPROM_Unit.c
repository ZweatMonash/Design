...
#include "EEPROM_Unit.h"


/*******************************************************************************
* Function Name: EEPROM_Unit_Enable
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Unit_Start
********************************************************************************
*
* Summary:
*  Starts EEPROM.
...
/*******************************************************************************
* Function Name: EEPROM_Unit_Stop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Unit_WriteByte
********************************************************************************
*
* Summary:
...
*  call EEPROM_Unit_UpdateTemperature() function if the temperature of the
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    EEPROM_Unit_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: EEPROM_Unit_ReadByte
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Unit_UpdateTemperature
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Unit_EraseSector
********************************************************************************
*
* Summary:
...
cystatus EEPROM_Unit_EraseSector(uint8 sectorNumber) 
{
    ...
    if(sectorNumber < (uint8) EEPROM_Unit_SECTORS_NUMBER)
    {
...
/*******************************************************************************
* Function Name: EEPROM_Unit_Write
********************************************************************************
*
* Summary:
...
*  EEPROM_Unit_UpdateTemperature() function if the temperature of the
...
/*******************************************************************************
* Function Name: EEPROM_Unit_StartWrite
********************************************************************************
*
* Summary:
...
*  EEPROM_Unit_Query(). EEPROM_Unit_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until EEPROM_Unit_Query() detects that
...
*  you should call EEPROM_Unit_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: EEPROM_Unit_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with EEPROM_Unit_Query().
*  EEPROM_Unit_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until EEPROM_Unit_Query() detects that the erase is
...
/*******************************************************************************
* Function Name: EEPROM_Unit_Query
********************************************************************************
*
* Summary:
*  Checks the status of an earlier call to EEPROM_Unit_StartWrite() or
*  EEPROM_Unit_StartErase().
...
/*******************************************************************************
* Function Name: EEPROM_Unit_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                EEPROM_Unit_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
