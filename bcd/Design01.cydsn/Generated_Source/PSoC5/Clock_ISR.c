...
#if !defined(Clock_ISR__REMOVED) /* Check for removal by optimization */
...
void Clock_ISR_Start(void)
{
    ...
    /* Set the priority. */
    Clock_ISR_SetPriority((uint8)Clock_ISR_INTC_PRIOR_NUMBER);
...
{
    #ifdef Clock_ISR_INTERRUPT_INTERRUPT_CALLBACK
        Clock_ISR_Interrupt_InterruptCallback();
    #endif /* Clock_ISR_INTERRUPT_INTERRUPT_CALLBACK */ 
...
*   before the component has been started use Clock_ISR_StartEx instead.
* 
*   When defining ISR functions, the CY_ISR and CY_ISR_PROTO macros should be 
*   used to provide consistent definition across compilers:
*
...
void Clock_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Clock_ISR__INTC_NUMBER] = address;
}
...
cyisraddress Clock_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Clock_ISR__INTC_NUMBER];
}
...
*
*   Note calling Clock_ISR_Start or Clock_ISR_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after Clock_ISR_Start or Clock_ISR_StartEx has been called. 
...
uint8 Clock_ISR_GetState(void)
{
    ...
    return ((*Clock_ISR_INTC_SET_EN & (uint32)Clock_ISR__INTC_MASK) != 0u) ? 1u:0u;
}
