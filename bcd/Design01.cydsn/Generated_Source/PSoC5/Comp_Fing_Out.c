...
uint8 Comp_Fing_Out_initVar = 0u;

/* Internal functions definitoin */
static void Comp_Fing_Out_trimAdjust(uint8 nibble) ;

/* static Comp_Fing_Out_backupStruct  Comp_Fing_Out_backup; */
#if (CY_PSOC5A)
    static Comp_Fing_Out_LOWPOWER_BACKUP_STRUCT  Comp_Fing_Out_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_Fing_Out_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_Fing_Out_Init
********************************************************************************
*
* Summary:
...
void Comp_Fing_Out_Init(void) 
{
    ...
    Comp_Fing_Out_SetSpeed(Comp_Fing_Out_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_Fing_Out_DEFAULT_HYSTERESIS == 0u )
        Comp_Fing_Out_CR |= Comp_Fing_Out_HYST_OFF;
    #else
        Comp_Fing_Out_CR &= (uint8)(~Comp_Fing_Out_HYST_OFF);
    #endif /* Comp_Fing_Out_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_Fing_Out_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_Fing_Out_CR &= (uint8)(~Comp_Fing_Out_PWRDWN_OVRD);
        #else 
            Comp_Fing_Out_CR |= Comp_Fing_Out_PWRDWN_OVRD;
        #endif /* Comp_Fing_Out_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_Fing_Out_CR |= Comp_Fing_Out_MX_AO;

    /* Set default sync */
    Comp_Fing_Out_CLK &= (uint8)(~Comp_Fing_Out_SYNCCLK_MASK);
    #if ( Comp_Fing_Out_DEFAULT_BYPASS_SYNC == 0u )
        Comp_Fing_Out_CLK |= Comp_Fing_Out_SYNC_CLK_EN;
    #else
        Comp_Fing_Out_CLK |= Comp_Fing_Out_BYPASS_SYNC;
    #endif /* Comp_Fing_Out_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_Fing_Out_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_Fing_Out_PWRMGR |= Comp_Fing_Out_ACT_PWR_EN;
    Comp_Fing_Out_STBY_PWRMGR |= Comp_Fing_Out_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_Fing_Out_restoreReg == 1u)
        {
            Comp_Fing_Out_CR = Comp_Fing_Out_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_Fing_Out_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Comp_Fing_Out_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_Fing_Out_initVar: Is modified when this function is called for the 
...
void Comp_Fing_Out_Start(void) 
{
    ...
    if ( Comp_Fing_Out_initVar == 0u )
    {
        Comp_Fing_Out_Init();
        
        Comp_Fing_Out_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Comp_Fing_Out_Stop
********************************************************************************
*
* Summary:
...
void Comp_Fing_Out_Stop(void) 
{
    ...
    Comp_Fing_Out_PWRMGR &= (uint8)(~Comp_Fing_Out_ACT_PWR_EN);
    Comp_Fing_Out_STBY_PWRMGR &= (uint8)(~Comp_Fing_Out_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_Fing_Out_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_Fing_Out_lowPowerBackup.compCRReg = Comp_Fing_Out_CR;
        Comp_Fing_Out_CR = Comp_Fing_Out_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_Fing_Out_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_Fing_Out_SetSpeed(uint8 speed) 
{
    ...
    Comp_Fing_Out_CR = (Comp_Fing_Out_CR & (uint8)(~Comp_Fing_Out_PWR_MODE_MASK)) |
                           (speed & Comp_Fing_Out_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_Fing_Out_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Fing_Out_TR = Comp_Fing_Out_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Fing_Out_TR0 = Comp_Fing_Out_HS_TRIM_TR0;
            Comp_Fing_Out_TR1 = Comp_Fing_Out_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Fing_Out_TR = Comp_Fing_Out_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Fing_Out_TR0 = Comp_Fing_Out_LS_TRIM_TR0;
            Comp_Fing_Out_TR1 = Comp_Fing_Out_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_Fing_Out_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_Fing_Out_WRK & Comp_Fing_Out_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_Fing_Out_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_Fing_Out_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_Fing_Out_WRK & Comp_Fing_Out_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Fing_Out_TR |= Comp_Fing_Out_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Fing_Out_TR0 |= Comp_Fing_Out_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Fing_Out_TR |= Comp_Fing_Out_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Fing_Out_TR1 |= Comp_Fing_Out_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        else
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Fing_Out_TR += 0x10u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Fing_Out_TR1 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        ...
        if ((Comp_Fing_Out_WRK & Comp_Fing_Out_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_Fing_Out_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_Fing_Out_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_Fing_Out_SW0;
    tmpSW2 = Comp_Fing_Out_SW2;
    tmpSW3 = Comp_Fing_Out_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_Fing_Out_SW0 = 0x00u;
    Comp_Fing_Out_SW2 = 0x00u;
    Comp_Fing_Out_SW3 = tmpSW3 & (uint8)(~Comp_Fing_Out_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Comp_Fing_Out_CR;
    Comp_Fing_Out_CR |= (Comp_Fing_Out_CAL_ON | Comp_Fing_Out_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_Fing_Out_TR = Comp_Fing_Out_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_Fing_Out_TR0 = Comp_Fing_Out_DEFAULT_CMP_TRIM;
        Comp_Fing_Out_TR1 = Comp_Fing_Out_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_Fing_Out_CR & Comp_Fing_Out_PWR_MODE_MASK) == Comp_Fing_Out_PWR_MODE_FAST)
    {
        Comp_Fing_Out_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_Fing_Out_trimAdjust(1u);
		Comp_Fing_Out_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Comp_Fing_Out_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Comp_Fing_Out_SW0 = tmpSW0;
    Comp_Fing_Out_SW2 = tmpSW2;
    Comp_Fing_Out_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_Fing_Out_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_Fing_Out_TR1 << 8u) | (Comp_Fing_Out_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_Fing_Out_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_Fing_Out_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_Fing_Out_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_Fing_Out_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
